...
class LNLN(nn.Module):
    ...
    def forward(self, complete_input, incomplete_input):
        ...
        # Completeness Check
        feat_tmp = self.completeness_check[0](h_1_l)[:, :1].squeeze() #Transformer 计算文本完整性特征
        w = self.completeness_check[1](feat_tmp) # completeness scores  一个全连接网络+Sigmoid 输出完整性得分 w

        #Proxy Dominant Feature Generator (generate h_1_p)
        h_0_p = repeat(self.h_p, '1 n d -> b n d', b = b)
        h_1_p = self.proxy_dominate_modality_generator(torch.cat([h_0_p, h_1_a, h_1_v], dim=1))[:, :8]
        h_1_p = self.GRL(h_1_p) #使用 梯度反转层 (GRL)，用来让 H^1_p 和真实文本特征尽可能相似
        ...
        h_d_list = self.dmml[0](h_1_d) #self.dmml[0]：Transformer 提取 H^1_d 的全局信息
        h_hyper = self.dmml[1](h_d_list, h_1_a, h_1_v, h_hyper) #self.dmml[1]：HhyperLearningEncoder 学习超模态特征
        
        #Cross-modality Fusion Transformer
        feat = self.dmml[2](h_hyper, h_d_list[-1]) #self.dmml[2] 是 跨模态 Transformer，融合 H^1_d 和超模态信息 h_hyper
        # Two ways to get the cls_output: using extra cls_token or using mean of all the features
        # output = self.dmml[3](feat[:, 0]) 
        output = self.dmml[3](torch.mean(feat[:, 1:], dim=1))#self.dmml[3] 最终全连接层，输出情感预测结果
        ...
        if (vision is not None) and (audio is not None) and (language is not None):
            ...
            # 重新计算 完整的文本、视觉、音频特征, 这些完整特征将 作为重构损失的监督信号，用于训练 Reconstructor
