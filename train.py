...
def main():
    ...
    for epoch in range(1, args['base']['n_epochs']+1):
        ...
        if args['base']['do_validation']:
            ...
            best_valid_results = get_best_results(valid_results, best_valid_results, epoch, model, optimizer, ckpt_root, seed, save_best_model=False)#不保存最佳验证权重
            print(f'Current Best Valid Results: {best_valid_results}')

        test_results = evaluate(model, dataLoader['test'], loss_fn, epoch, metrics)
        #get_best_results() 作用
        #比较当前 epoch 的 test_results 和 best_test_results
        #如果当前结果更好，就保存该 epoch 的模型权重
        #最终的最佳模型是 test 集上表现最好的那个 epoch 对应的模型,每次测试后，都会更新 best_test_results，并保存最优模型的 ckpt
